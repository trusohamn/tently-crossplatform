...
import { WebViewLeaflet, WebViewLeafletEvents, } from 'react-native-webview-leaflet';
...
const MapLeaflet = ({ markers = [], zoom: zoomSetting, position: positionSetting, selectedPosition, setSelectedPosition, markerIcon, }) => {
    ...
    const onMessageReceived = (message) => {
        ...
        switch (message.event) {
            ...
            case WebViewLeafletEvents.ON_MAP_MARKER_CLICKED:
                Alert.alert(` ${markers[((_b = message === null || message === void 0 ? void 0 : message.payload) === null || _b === void 0 ? void 0 : _b.mapMarkerID) - 1].name ||
                    'unknown'}`);
                break;
        }
    };
    const setMarkersOnMap = () => {
        const locationMarkers = markers.map((marker) => {
            return Object.assign(Object.assign({}, marker), { icon: Image.resolveAssetSource(marker.icon || '  ').uri });
        });
        !!selectedPosition &&
